...
def color_box(modelname, violation_class, cam_no,results, frame, classes, acc=0.45):
    prev_time = 0
        ...
        if cords_list[4] >= acc:  # Accuracy Comparison
            ...
            if text_d == violation_class:
                ...
                if current_time != prev_time or prev_time == 0:
                    ...
                    # print(modelpath)
                    image_link = ml_functions_copy.voilation_screenshot(frame, modelpath)  # Image Link
                    sqs_trigger_json = ml_functions_copy.json_generator(violation, cords_list, image_link,cam_no)  # JSON Output
