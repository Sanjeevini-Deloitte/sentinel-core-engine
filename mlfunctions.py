...
from storage_bucket import upload_screenshots,create_cf_url
...
def json_generator(text_d, cords_list,image_link):
    json_output.update({"cameraId": 1})
...
def cloudfront_link(model_path):
    link=create_cf_url(model_path+time.strftime("%H%M%S"))
    return link
...
def voilation_screenshot(frame, model_path):
    ...
    folder_path=config.get('aws','BUCKET_FOLDER')
    ...
    image_link = upload_screenshots(folder_path+model_path+time.strftime("%H%M%S"), screenshot_link,model_path+time.strftime("%H%M%S")) # Upload screenshot to S3 Bucket
